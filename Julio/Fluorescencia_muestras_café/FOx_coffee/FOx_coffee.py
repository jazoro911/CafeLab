...
datos = pd.read_csv("FOx_data.csv")
...
fig.update_layout(
    title="Curva de intensidad (CPS) - Muestra FOx",
...
fig.update_layout(
    title="Espectro de fluorescencia de la muestra de café de Oaxaca (FOx)", 
    xaxis_title="Longitud de onda (nm)",
    yaxis_title="Intensidad (CPS)",
    legend_title="Muestra",
    template="plotly_white",
...
<head>
  <meta charset="UTF-8">
  <title>Reporte de espectro de fluorescencia - Muestra FOx </title> 
...
<body>
  ...
  <h1>Reporte de espectro de fluorescencia - Muestra FOx</h1>
  ...
  <div>
    <p>Este espectro muestra el perfil de respuesta óptica de la muestra FOx, con análisis cuantitativo por zonas espectrales específicas. La información obtenida puede relacionarse con compuestos funcionales presentes en la muestra.</p>
  </div>
...
with open("Reporte_FOx.html", "w", encoding="utf-8") as f:
    f.write(html_final)

print("✅ Reporte generado: Reporte_FOx.html")
